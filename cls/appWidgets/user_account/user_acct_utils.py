...
from cls.utils.json_threadsave import (
    dict_to_json_string,
    json_string_to_dct,
    saveJson,
    loadJson,
...
class user_accnt_mgr(object):
    ...
    def add_user(
        self, username, password, group="visitor", access_lvl=ACCESS_LVLS.GUEST
    ):
    ...
    def load_file(self):
        if os.path.exists(self.user_accnt_file):
            ...
            if js:
                ...
                _logger.error(
                    "unable to load user account file [%s]" % self.user_accnt_file
                )
        else:
