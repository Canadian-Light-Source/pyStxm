...
def gen_axis_dict(roi, prioritize_center=False):
    """
    "axes": [
        {
            "nPoints": 10
    ...
    r["positionerName"] = app_to_dcs_devname_map[roi[POSITIONER]]
    dct["trajectories"].append(r)

    return dct

def gen_axis_dicts(axis_str_lst, rois):
    dct = {}
    axis_zip = zip(axis_str_lst, rois)
    for axis_str, roi in axis_str_lst:
        dct[axis_str] = gen_axis_dict(roi)

    return dct

def gen_regions(scan_type_str, dwell, parent, is_outer=False):
    """
    ...
    regs = gen_region_dict(dwell, region_lst, add_axes, is_outer, scan_type_str=scan_type_str)
    if type(regs) == list:
        if not is_outer:
            for r in regs:
                regions.append(r)
...
def gen_region_dict(dwell: float, rois: [dict], add_axes=False, is_outer=False, scan_type_str='') -> dict:
    """
        ...
        if is_outer:
            ...
            for roi in rois:
                ...
                if is_outer:
                    dct["dwellTime"] = dwell
                dct["axes"] = []
                dct["axes"].append(gen_axis_dict(roi, prioritize_center=prioritize_center))
                lst.append(dct)
            return lst
        else:
            #inner
...
def gen_displayed_axis_dict(inner=True, index=0):
    if inner:
        region_str = "inner"
    else:
        region_str = "outer"
...
def gen_displayed_axis_dicts(axis_str_lst, axis_rois, inner_outer_lst):
    dct = {}
    axis_zip = zip(axis_str_lst, axis_rois,inner_outer_lst)
    index = 0
    for axis_str, roi, inout in axis_zip:
        dct[axis_str] = gen_displayed_axis_dict(inout, index)
        index += 1
    return dct

def gen_point_displayed_axis_dicts(axis_str_lst, axis_rois, inner_outer_lst):
    dct = {}
    axis_zip = zip(axis_str_lst, axis_rois,inner_outer_lst)
    index = -1
    for axis_str, roi, inout in axis_zip:
...
def gen_base_req_structure(scan_type_str: str) -> dict:
    """
...
def convert_positioner_name(roi: dict) -> str:
    """
...
def make_base_energy_region(e_roi, app_to_dcs_devname_map):
    dct = {
                ...
                "trajectories": [
                    {
                        ...
                        "range": e_roi['RANGE'],
                        "step": e_roi['STEP'],
                        "positionerName": app_to_dcs_devname_map['DNM_ENERGY']
                    }
                ]
            }
        ]
...
def make_point_spatial_region(sp_roi_0, sp_roi_1, app_to_dcs_devname_map):
    dct = {
            "axes": [
                {
                    ...
                    "trajectories": [
                        {
                            ...
                            "positionerName": app_to_dcs_devname_map[sp_roi_0['POSITIONER']]
                        },
                        {
                            "center": sp_roi_1['CENTER'],
                            "range": sp_roi_1['RANGE'],
                            "positionerName": app_to_dcs_devname_map[sp_roi_1['POSITIONER']]
                        }
                    ]
                }
            ]
