            ...
            try:
                ...
                if dct["count"] > 1:
                    body[str(i)] = pvproperty(
                        name=k, dtype=data_type, max_length=dct["count"], value=val
                    )
                else:
                    body[str(i)] = pvproperty(
                        name=k, dtype=data_type, mock_record=rec_type, value=val
                    )
                ...
                for fld in list(mtr_dct[pv_nm].keys()):
                    if len(fld) > 0:
                        ...
                        if fld.find("MSTA") > -1:
                            ...
                            dct[DEV_PRFX + pv_nm + ".%s" % fld]["value"] = mtr_dct[
                                pv_nm
                            ][fld]
                        # print('serving MOTOR: [%s], value=[%s]' % ('SIM_' + pv_nm + '.%s' % fld, dct['SIM_' + pv_nm + '.%s' % fld]['value']))
                        print(
                            "serving MOTOR: [%s], value=[%s]"
                            % (
                                DEV_PRFX + pv_nm + ".%s" % fld,
                                dct[DEV_PRFX + pv_nm + ".%s" % fld]["value"],
                            )
                        )
            else:
                dct[DEV_PRFX + pv_nm] = jdct[pv_nm]
