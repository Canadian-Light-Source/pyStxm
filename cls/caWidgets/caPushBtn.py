...
class caPushBtnWithFbk(QtWidgets.QPushButton):
    ...
    def change_btn_color(self, val):
        ...
        if val == 0:
            # in=0
            s = format_btn(title_color="white", bgcolor=master_colors["btn_pressed"]["rgb_str"])
        elif val == 1:
            # out =1
            s = format_btn(title_color="black", bgcolor=master_colors["btn_pressed"]["rgb_str"])
        else:
            # moving
...
class test(QtWidgets.QWidget):
    ...
    def __init__(self):
        ...
        self.mode_pb = caPushBtn(
            fl_mode_dev,
            off_val=1,
            on_val=0,
            off_str="Sample Focused",
            on_str="OSA Focused",
        )

        discon_dev = BaseDevice("BL1610-I10:ENERGY:uhv:zp:scansel")
        # pv, init_val, off_val, on_val, off_str, on_str, cb=None):
        self.discon_pb = caPushBtn(
            discon_dev, off_val=0, on_val=1, off_str="Some PV Off", on_str="Some PV On"
        )

        zp_inout_dev = BaseDevice("BL1610-I10:uhv:zp_inout")
        zp_inoutfbk_dev = Mbbi("BL1610-I10:uhv:zp_inout:fbk")
        # pv, init_val, off_val, on_val, off_str, on_str, cb=None):
        self.zp_pb = caPushBtnWithFbk(
            zp_inout_dev,
            sig_change_kw="char_value",
            off_val=0,
            on_val=1,
            off_str="ZP_IN",
            on_str="ZP_OUT",
            fbk_dev=zp_inoutfbk_dev,
            toggle=False,
        )
