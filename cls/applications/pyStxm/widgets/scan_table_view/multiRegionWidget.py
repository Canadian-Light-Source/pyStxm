...
from cls.applications.pyStxm.widgets.scan_table_view.spatialSelWidget import (
    SpatialSelWidget,
)
...
class MultiRegionWidget(BaseSelectionWidget):
    ...
    ev_total_changed = QtCore.pyqtSignal()
    ...
    def __init__(
        ...
        *args
        ...
        self.setGeometry(300, 200, 570, 850)
        ...
        self.ev_widg.total_ev_changed.connect(self.on_ev_total_changed)
        ...
        self.sp_widg = SpatialSelWidget(
            self.ev_widg,
            use_center=use_center,
            is_point=is_point,
            is_arb_line=is_arb_line,
            single_ev_model=single_ev_model,
            max_range=max_range,
            use_hdw_accel=use_hdw_accel,
            single_sp_region=single_sp,
            min_sp_rois=min_sp_rois,
            x_cntr=x_cntr,
            y_cntr=y_cntr,
            scan=sp_scan
        )
        ...
        single_ev_lbl = QtWidgets.QLabel("Energy")
        ...
        self.exec_single_ev_chkbx.clicked.connect(self.on_exec_single_ev)

        if self.main_obj:
            ev = self.main_obj.device("DNM_ENERGY").get_position()
            self.singleEvFld.setText(f"{ev:.2f}")

        s_ev_h_layout = QtWidgets.QHBoxLayout()
        s_ev_h_layout.addItem(spacer)
        s_ev_h_layout.addWidget(single_ev_lbl)
        s_ev_h_layout.addWidget(self.singleEvFld)
        s_ev_h_layout.addItem(spacer)
        s_ev_h_layout.addWidget(single_dwell_lbl)
        s_ev_h_layout.addWidget(self.singleDwellFld)
        s_ev_h_layout.addItem(spacer)
        s_ev_h_layout.addWidget(self.exec_single_ev_chkbx)

        h_layout.addItem(spacer)
        h_layout.addWidget(self.ev_total_lbl)
        h_layout.addItem(spacer)

        v_layout.addLayout(h_layout)
        v_layout.addLayout(s_ev_h_layout)
        ...
        if enable_multi_spatial:
            ...
            self.on_multi_spatial_chk(False)
            # trim size of the single spatial region widget and give proper space to the other widgets
            self.sp_widg.setMaximumHeight(70)
            self.sp_widg.setMinimumHeight(70)
            self.ev_widg.setMinimumHeight(100)
            self.pol_widg.setMinimumHeight(60)

        self.ev_widg.set_table_to_editable(True)
        self.pol_widg.set_table_to_editable(True)
    ...
    def get_multi_region_widget_scan_time_params(self):
        """
        return the required scan params to estimate scan time
        """

        dwells = self.get_just_dwells()
        sp_rois = self.get_just_sp_regions()
        ev_rois = self.get_just_ev_regions()
        pol_rois = self.get_just_pol_regions()

        dwell = dwells[0]
        nx = sp_rois[0]['X']['NPOINTS']
        ny = sp_rois[0]['Y']['NPOINTS']
        npoints_ev = ev_rois[0]['NPOINTS']
        npoints_pol = len(pol_rois)

        return {'dwell': dwell, 'npoints_x': nx, 'npoints_y': ny, 'npoints_ev': npoints_ev, 'npoints_pol': npoints_pol}

    def is_set_for_single_ev(self):
        """
        used by parents of this widget to know if single ev is set or not
        Returns: bool

        """
        return self.exec_single_ev_chkbx.isChecked()

    def get_single_ev_value(self):
        """
        used by parents of this widget to retrieve the value of the single_ev_fld
        Returns: float

        """
        return float(self.singleEvFld.text())

    def get_single_dwell_value(self):
        """
        used by parents of this widget to retrieve the value of the dwell_fld
        Returns: float

        """
        return float(self.singleDwellFld.text())

    def on_ev_total_changed(self, num):
        """
        update the total ev label
        Args:
            num:
        ...
        self.ev_total_lbl.setText(f"Number of energy points: {int(num)}")
        self.ev_total_changed.emit()

    def on_exec_single_ev(self, chkd):
        """
        a function to handle setting the tables accordingly so that the widget returns a single ev scan but
        doesnt change the contents of any of the ev fields.

        If this is checked the eV value to use will be located in the START field
        ...
        """
        if chkd:
            #need to change the look of the rows in all widgets
            self.on_ev_total_changed(1)
            self.singleEvFld.setEnabled(True)
            self.singleDwellFld.setEnabled(True)
    ...
    def disable_polarization_table(self, val):
        if val:
            ...
            self.pol_widg.setMaximumHeight(60)
        else:
            self.pol_widg.setEnabled(True)
    ...
    def on_load_scan(self):
        ...
        fname = getOpenFileName(
            "Load Scan",
            filter_str="Scan Files (%s*.hdf5)" % data_file_pfx,
            search_path=datadir,
        )
        self.openfile(fname)
    ...
    def load_scan(self, wdg_com, append=False, ev_only=False, sp_only=False):
        ...
        multi_scan_types = [
            ...
            scan_types.SAMPLE_IMAGE + IMAGE_LXL,
            scan_types.SAMPLE_IMAGE + IMAGE_PXP,
        ...
        if scan_type not in multi_scan_types:
            _logger.error(
                "Incorrect scan type [%d], not a multi spatial region scan" % scan_type
            )
            return
        ...
        #now update the total number of ev points label
        if self.is_set_for_single_ev():
    ...
    def set_spatial_table_visible(self, val):
        self.sp_widg.setVisible(val)

    def set_energy_table_visible(self, val):
        self.ev_widg.setVisible(val)
    ...
    def on_spatial_model_changed(self, scan_id):
        ...
        scan = self.sp_widg.get_row_data_by_item_id(scan_id)
        if scan is not None:
            x_roi = scan[SPDB_X]
            y_roi = scan[SPDB_Y]
    ...
    def get_just_ev_regions(self, do_print=False):
        ...
        """
        evs = []
        if self.exec_single_ev_chkbx.isChecked():
            evs.append(self.return_single_ev_region())
        else:
            data = self.ev_widg.get_all_data()
            for d in data:
                evs.append(d)
    ...
    def get_just_dwells(self, do_print=False):
        ...
        if self.exec_single_ev_chkbx.isChecked():
            dwells.append(self.get_single_dwell_value())
        else:
    ...
    def return_single_ev_region(self):
        """
        This function is called to turn a multi ev roi into a single because th euser has selected "Execute single Ev"
        Args:
            e_roi:

        Returns: an energy roi that used the single values specified frr energy and dwell

        """
        pol_rois = self.pol_widg.get_all_data()
        pol_roi = [pol_rois[0]]
        ev = self.get_single_ev_value()
        dwell = self.get_single_dwell_value()
        new_ev_roi = get_base_energy_roi(
            "EV", "DNM_ENERGY", start=ev, stop=ev, rng=0.0, npoints=1, dwell=dwell, pol_rois=pol_roi
        )
    ...
    def print_sp_roi(self, roi):
        ...
        """
        print(
            "Spatial ROI [%d] CenterX[%.2f] RangeX[%.2f] CenterY[%.2f] RangeY[%.2f] EV_MODEL=%d"
            % (
                roi[SPDB_ID_VAL],
                roi["X"][CENTER],
                roi["X"][RANGE],
        ...
        print(
            "Spatial ROI [%d] StartX[%.2f] StopX[%.2f] StartY[%.2f] StopY[%.2f]"
            % (
                roi[SPDB_ID_VAL],
                roi["X"][START],
                roi["X"][STOP],
    ...
    def print_ev_roi(self, roi):
        ...
        """
        print(
            "\tEV ROI [%d] Start[%.2f] End[%.2f] Range[%.2f] Points[%.2f] Dwell[%.2f] POL_MODEL=%d"
            % (
                roi[SPDB_ID_VAL],
                roi[START],
                roi[STOP],
    ...
    def print_pol_roi(self, ev_setpoint, roi):
        ...
        """
        print(
            "\t\tPOL ROI [%d] [%.3f ev]  POL[%d] Offset[%.2f] Angle[%.2f]"
            % (roi[SPDB_ID_VAL], ev_setpoint, roi[POL], roi[OFF], roi[ANGLE])
        )

    def resize_tableviews(self):
        """
        force all of the selection widgets tables views ro resize to their contents

        NOTE: for this to work the shopw() function must have already been called in order for the
        contents sizes to be calculated correctly

        """
        self.resize_tableview(self.sp_widg)
        self.resize_tableview(self.ev_widg)
        self.resize_tableview(self.pol_widg)


    def resize_tableview(self, parent_wdg):
        """
        take a table view that is currently visible and force it to reclac its sizes
        """
        tbl_view = parent_wdg.table_view
        orig_size = tbl_view.size()  # New size after "resize"
        new_size = orig_size
        new_size.setWidth(orig_size.width() + 1)
        event = QtGui.QResizeEvent(new_size, orig_size)
        tbl_view.resizeEvent(event)
...
if __name__ == "__main__":
    ...
    multi = MultiRegionWidget(
        use_center=True,
        enable_multi_spatial=True,
        is_point=False,
        single_ev_model=True,
        single_pol_model=True,
        min_sp_rois=1,
    )
    #multi.setFixedWidth(600)
    # multi.disable_polarization_table(True)
    # multi.enable_add_spatial_region_menu(True)
    multi.show_load_btn()
    multi.show_getdata_btn()

    # can control which tables we see
    # multi.set_spatial_table_visible(False)
    # multi.set_energy_table_visible(False)
    # multi.set_polarization_table_visible(True)
    ss = get_style('dark')
    multi.setStyleSheet(ss)
