...
from bcm.devices.ophyd.e712_wavegen.e712_errors import e712_errors
...
def gen_seg_str(
    tblid, start, stop, dwell=1.0, num_wv_points=10, accRange=2, _new=False
):
    if start < 0:
...
def plot_data(xdat, ystr_dat):
    ...
    x = 0
    for y in ydat:
        plot.add_xy_point("point_spectra", x, y, update=False)
        x += 1
    plot.add_xy_point("point_spectra", x, y, update=True)
    plot.show()
...
def gen_y_step_wav_strs(start, stop, step, npoints, dwell, do_clear=True, tbl_id=2):
    ...
    pos = start
    l.append(
        gen_seg_str(1, pos, pos, dwell=dwell, num_wv_points=20, accRange=0, _new=True)
    )
    ...
    for i in range(npoints):
        ...
        l.append(
            gen_seg_str(tbl_id, pos, pos, dwell=dwell, num_wv_points=20, accRange=1)
        )
        # move to next step
        l.append(
            gen_seg_str(tbl_id, pos, pos + _step, dwell=1, num_wv_points=5, accRange=1)
        )
        pos += _step
    ##stay
...
def define_seg_by_time(
    seg_time, speedupdown_time, step_size, offset, _new=True, tblid=1
):
...
def gen_y_line_wav_strs(
    start, stop, step, step_time, sit_time, do_clear=False, tbl_id=2
):
    """ """
    l = []
    if do_clear:
...
if __name__ == "__main__":
    ...
    trig_str = gen_pxp_line_trig_str(dwell=1.0, npoints=50, accRange=1.0, velo=50.0)
