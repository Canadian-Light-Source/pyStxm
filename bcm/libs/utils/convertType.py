...
def convertToCtype(arg, extraTypesModule):
    ...
    dots = re.compile("\.\.\.")
    call_back = re.compile(
        r"([^\s]*CallbackPtr)\s*([^\s]*)"
    )  # Match "DAQmxDoneEventCallbackPtr callbackFunction"
    if const_char.search(arg):
        ...
        if (
            pointer_type2.search(arg).group(2) == "readArray"
            or pointer_type2.search(arg).group(2) == "writeArray"
        ):
