...
class BaseCoarseImageScanParam(ScanParamWidget):
    def __init__(
        ...
        self._parent = parent
        uic.loadUi(
            os.path.join(ui_path, "coarse_image_scan.ui"), self
        )
        ...
        if self.sample_positioning_mode == sample_positioning_modes.GONIOMETER:
            ...
            self.setToolTip(
                "CoarseImageScanParam: Scan plugin is disabled while in Goniometer sample positioning mode"
            )
        else:
            ...
            # self.scan_class = self.instanciate_scan_class(__file__, 'CoarseSampleImageScan', 'CoarseSampleImageScanClass')
    ...
    def gen_max_scan_range_limit_def(self):
        ...
        bounding = ROILimitObj(
            bounding_qrect,
            get_alarm_clr(255),
            "Range is beyond Sample Coarse Capabilities",
            get_alarm_fill_pattern(),
        )
        normal = ROILimitObj(
            bounding_qrect,
        ...
        warn = ROILimitObj(
            warn_qrect,
            get_warn_clr(150),
            "Nearing max Range of Sample Coarse X/Y",
            get_warn_fill_pattern(),
        )
        alarm = ROILimitObj(
            alarm_qrect,
            get_alarm_clr(255),
            "Beyond range of Sample Coarse X/Y",
            get_alarm_fill_pattern(),
        )
    ...
    def update_last_settings(self):
        ...
        DEFAULTS.set(
            "SCAN.%s.CENTER" % self.section_id, (x_roi[CENTER], y_roi[CENTER], 0, 0)
        )
        DEFAULTS.set(
            "SCAN.%s.RANGE" % self.section_id, (x_roi[RANGE], y_roi[RANGE], 0, 0)
        )
        DEFAULTS.set(
            "SCAN.%s.NPOINTS" % self.section_id, (x_roi[NPOINTS], y_roi[NPOINTS], 0, 0)
        )
