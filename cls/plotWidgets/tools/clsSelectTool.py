...
from plotpy.events import ObjectHandler, KeyEventMatch
...
def _setup_standard_tool_filter(_filter, start_state):
    ...
    # Bouton droit
    ZoomHandler(_filter, QtCore.Qt.MouseButton.RightButton, start_state=start_state)
    MenuHandler(_filter, QtCore.Qt.MouseButton.RightButton, start_state=start_state)
...
class clsMoveHandler(object):
    ...
    def __init__(self, _filter, btn=Qt.NoButton, mods=Qt.NoModifier, start_state=0):
        _filter.add_event(
            start_state,
            _filter.mouse_move(Qt.NoButton, Qt.NoModifier),
            self.move,
            start_state,
        )
        _filter.add_event(
            start_state, _filter.mouse_move(btn, mods), self.move, start_state
        )
        # add event for emitting the center has changed
        _filter.add_event(
            start_state,
            KeyEventMatch(((Qt.Key_C, Qt.ControlModifier),)),
            self.on_keypress,
            start_state,
        )
...
class clsSelectTool(InteractiveTool):
    ...
    def setup_filter(self, baseplot):
        ...
        ObjectHandler(_filter, QtCore.Qt.MouseButton.LeftButton, start_state=start_state)
        ObjectHandler(
            _filter,
        ...
        self.s = clsMoveHandler(
            _filter, btn=QtCore.Qt.MouseButton.LeftButton, mods=Qt.ControlModifier, start_state=start_state
        )
