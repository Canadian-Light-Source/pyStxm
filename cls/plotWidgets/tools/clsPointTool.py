...
from plotpy.items.annotation import AnnotatedPoint
...
class clsPointTool(AnnotatedPointTool):
    ...
    def activate(self, checked=True):
        ...
        # click of the tool
        p = get_parent_who_has_attr(self.manager, 'deselect_all_shapes')
        if p:
            p.deselect_all_shapes()
            if isinstance(self.sender(), QtWidgets.QAction) or isinstance(self.sender(), QtWidgets.QPushButton):
                self.shapeNum += 1
