                ...
                if self.warn.exceeds(qrect):
                    ...
                    if self.alarm:
                        if self.alarm.exceeds_range(qrect):
                            ...
                            self.state_qrect = self._limit_qrect(
                                self.alarm.get_qrect(), qrect
                            )
        else:
...
if __name__ == "__main__":
    ...
    bounding = ROILimitObj(
        bounding_qrect, get_alarm_clr(255), "Out of Bounds", get_normal_fill_pattern()
    )
    normal = ROILimitObj(
        QRectF(QPointF(0, 0), QPointF(100, -100)),
        get_normal_clr(25),
        "All is Normal",
        get_normal_fill_pattern(),
    )
    warn = ROILimitObj(
        QRectF(QPointF(0, 0), QPointF(250, -250)),
        get_warn_clr(150),
        "Warning",
        get_warn_fill_pattern(),
    )
    alarm = ROILimitObj(
        QRectF(QPointF(0, 0), QPointF(500, -500)),
        get_alarm_clr(255),
        "ALARM the selectoin box is too larg",
