...
class EnergyScanTableModel(BaseScanTableModel):
    def __init__(self, hdrList, datain, single_dwell=False, parent=None, *args):
        """
    ...
    def setData(self, index: QtCore.QModelIndex, value: Any, role: QtCore.Qt.ItemDataRole, recalc=True, do_signal=True):
        ...
        if role == QtCore.Qt.EditRole:
            ...
            if col == 0:
                ...
                if self.single_dwell:
                    # the first row dwell is the master
                    if row == 0:
                        #master changed
                        val = inst_clss(value)
                        # set all existing energy regions to use master dwell
                        self.set_all_scans_to_master_dwell(val)
                    else:
                        #force other energy regions to use master dwell
                        val = inst_clss(self.scanListData[0][DWELL])
                else:
    ...
    def set_all_scans_to_master_dwell(self, master_dwell):
        """
        force all energy regions dwell times to the master
        """
        for scan in self.scanListData:
            scan[DWELL] = master_dwell
    ...
    def recalc_params(self, row=None):
        ...
        for idx, scan in enumerate(self.scanListData[row:], start=row):
            if idx == 0:
                pass
            else:
