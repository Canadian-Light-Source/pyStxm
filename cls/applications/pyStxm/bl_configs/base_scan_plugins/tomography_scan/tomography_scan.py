...
TOMOGRAPHY_ENABLED = MAIN_OBJ.get_preset_as_bool("TOMOGRAPHY_ENABLED", "BL_CFG_MAIN")


class BaseTomographyScanParam(MultiRegionScanParamBase):
    def __init__(
        ...
        uic.loadUi(os.path.join(ui_path, "tomography_scan.ui"), self)

        # if (self.sample_positioning_mode != sample_positioning_modes.GONIOMETER):
        gx = self.main_obj.device("DNM_GONI_X")
        gy = self.main_obj.device("DNM_GONI_Y")
        gt = self.main_obj.device("DNM_GONI_THETA")

        if not TOMOGRAPHY_ENABLED or (gt is None):
            if TOMOGRAPHY_ENABLED:
                self.name = "Tomography Scan ---- [DISABLED: No Goniometer Theta in Device configuration] "
                self.setToolTip(
                    "TomographyScansParam: Scan plugin is disabled as there is no rotational stage (Goniometer) in Devices"
                )
            else:
                self.name = "Tomography Scan ---- [DISABLED by scanning mode] "
                self.setToolTip(
                    "TomographyScansParam: Scan plugin is disabled while in Goniometer sample positioning mode"
                )
            ...
            self.tomo_zpz_adjust_wdg = uic.loadUi(
                os.path.join(os.path.dirname(__file__), "tomo_zpz_adjust.ui")
            )
            self.tomo_zpz_adjust_wdg.setModal(True)

            x_cntr = gx.get_position()
            y_cntr = gy.get_position()
            ...
            self.tomo_zpz_adjust_wdg.zpzEnableAdjustChkBox.clicked.connect(
                self.on_zpz_enable_chkbox
            )
            # self.useE712WavegenBtn.clicked.connect(self.on_E712WavegenBtn)

            # self.scan_class = self.instanciate_scan_class(__file__, 'TomographyWithE712WavegenScan', 'TomographyWithE712WavegenScanClass')
    ...
    def init_plugin(self):
        ...
        self._type_do_recenter = True  # [scan_panel_order.IMAGE_SCAN, scan_panel_order.TOMOGRAPHY, scan_panel_order.LINE_SCAN]
    ...
    def calc_new_scan_time_estemate(self, is_pxp, x_roi, y_roi, dwell_in_sec):
        ...
        if True:
            ...
            hline_time = self.scan_class.e712_wg.calc_hline_time(
                dwell_in_sec, x_roi[NPOINTS], pxp=pxp
            )
    ...
    def load_roi(self, wdg_com, append=False, ev_only=False, sp_only=False):
        ...
        if self.wdg_com[CMND] == widget_com_cmnd_types.LOAD_SCAN:
            ...
            if (not ev_only) and (
                dct_get(sp_db, SPDB_SCAN_PLUGIN_TYPE) not in image_scans
            ):
                return
            ...
            self.enable_multi_region = (
                self.multi_region_widget.is_multi_region_enabled()
            )
        ...
        if self.wdg_com is not None:
            ...
            for sp_id in sp_ids:
                ...
                if USE_E712_HDW_ACCEL:
                    ...
                    dct_put(
                        sp_db, SPDB_HDW_ACCEL_AUTO_DDL, self.autoDDLRadBtn.isChecked()
                    )
                    dct_put(
                        sp_db,
                        SPDB_HDW_ACCEL_REINIT_DDL,
                        self.reinitDDLRadBtn.isChecked(),
                    )
                ...
                if dct_get(sp_db, SPDB_EV_NPOINTS) > 1:
                    self.type = scan_types.TOMOGRAPHY
                    dct_put(
                        sp_db, SPDB_SCAN_PLUGIN_TYPE, scan_types.TOMOGRAPHY
