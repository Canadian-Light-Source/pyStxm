...
class BaseFineSampleImageScanClass(BaseScan):
    """
    ...
    #def make_lxl_scan_plan(self, dets, gate, md={}, bi_dir=False):
    def make_lxl_scan_plan(self, dets, md=None, bi_dir=False):
        ...
        def do_scan():
            ...
            # # go to start of line
            # yield from bps.mv(
            #     mtr_x, self.x_roi[START] - ACCEL_DISTANCE, mtr_y, self.y_roi[START]
            # )
            #
            # # now do a horizontal line for every new zoneplate Z setpoint
            # for y_sp in self.y_roi[SETPOINTS]:
            #     yield from bps.mv(
            #         mtr_y, y_sp, mtr_x, self.x_roi[START] - ACCEL_DISTANCE
            #     )
    ...
    def configure(
        ...
        """
        super().configure(wdg_com, sp_id=sp_id, line=line, z_enabled=False)
