...
class BaseDetectorScanParam(ScanParamWidget):
    ...
    def __init__(
        ...
        self._parent = parent
        uic.loadUi(os.path.join(ui_path, "det_scan.ui"), self)
        self.scan_mod_path, self.scan_mod_name = self.derive_scan_mod_name(__file__)

        # self.scan_class = self.instanciate_scan_class(__file__, 'DetectorScan','DetectorScanClass')

        # self.scribblerBtn.setVisible(True)
        # self.scribblerBtn.setToolTip(
        #     "Click the image and then hold down CTRL-C to move detector \nstage around looking for the beam"
        # )
        ...
        #depending on the stxm configuration, osa out might be towards the high or low limit
        self.osa_out_limit = "high" #can be `high` or `low`, default is `high`
        self.sp_db = None
    ...
    def init_plugin(self):
        ...
        self._help_html_fpath = os.path.join(
            "interface", "window_system", "scan_plugins", "detector.html"
        )
    ...
    def gen_max_scan_range_limit_def(self):
        ...
        bounding = ROILimitObj(
            bounding_qrect,
            get_alarm_clr(255),
            "Range is beyond Detector Capabilities",
            get_alarm_fill_pattern(),
        )
        normal = ROILimitObj(
            bounding_qrect,
        ...
        warn = ROILimitObj(
            warn_qrect,
            get_warn_clr(150),
            "Nearing max Range of Detector X/Y",
            get_warn_fill_pattern(),
        )
        alarm = ROILimitObj(
            alarm_qrect,
            get_alarm_clr(255),
            "Beyond range of Detector X/Y",
            get_alarm_fill_pattern(),
        )
    ...
    def on_set_center(self):
        """
        when user has clicked set center button
        """
    ...
    def on_do_scribbler(self, checked):
        if checked:
            ...
            self.plotWidget.initData(
                image_types.IMAGE,
                sizex * 2.0,
                sizey * 2.0,
                {SPDB_RECT: (-1 * sizex, -1 * sizey, sizex, sizey)},
            )
