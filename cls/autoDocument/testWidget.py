...
# from plotpy.io import pil_to_array
...
from plotpy.items.annotation import AnnotatedPoint, AnnotatedRectangle
from guiqwt.config import _
from plotpy.items.annotation import EllipseShape
from plotpy.items.annotation import LabelItem
...
class SquareAspectRatioTool(ToggleTool):
    ...
    def __init__(
        self, manager, plot, icon="aspect.png", tip="Force aspect Ratio to be square"
    ):
        super(SquareAspectRatioTool, self).__init__(
            manager, _("Force aspect ratio to a square"), icon
        )
        self.plot = plot
...
class StxmImageWidget(ImageDialog):
    ...
    def __init__(
        self, parent, filtStr="*.json;*.hdr", testing=False, type="basic", options=None
    ):
        ...
        ImageDialog.__init__(
            self, parent=parent, wintitle="", edit=False, toolbar=True, options=options
        )
        ...
        self.connect(
            self.plot, SIGNAL("MouseButtonRelease(QMouseEvent)"), self.mouseReleaseEvent
        )
    ...
    def register_select_tools(self):
        ...
        ap = self.add_tool(
            apt, setup_shape_cb=self.setuppoint, handle_final_shape_cb=self.newpoint
        )
    ...
    def initData(self, prefix, rows, cols, parms={}):
        ...
        self.set_image_parameters(
            self.item, parms["xmin"], parms["xmax"], parms["ymin"], parms["ymax"]
        )
    ...
    def clear_data(self, prefix, rows, cols):
        ...
        if self.item is None:
            self.item = make.image(
                self.data, interpolation="linear", colormap="gist_gray"
            )
            # self.item = make.image(self.data, interpolation='nearest', colormap='gist_gray')
    ...
    def loadFilterPanel(self, plotter):
        ...
        self.connect(
            filterPanel.filterSlider, SIGNAL("valueChanged(int)"), self.apply_filter
        )
        self.connect(
            filterPanel.filterComboBox,
            SIGNAL("currentIndexChanged(int)"),
            self.onFilterIndexChanged,
        )
        return filterPanel
    ...
    def openfile(self, fileName, addimages=True):
        ...
        if (not addimages) or (self.item is None):
            ...
            # self.item = make.image(self.data, interpolation='nearest', colormap='gist_gray')
            self.item = make.image(
                self.data, interpolation="linear", colormap="jet", title=str(fileName)
            )
            plot = self.get_plot()
            plot.add_item(self.item, z=0)
            # plot.set_plot_limits(0.0, 740.0, 0.0, 800.0)
        ...
        self.set_image_parameters(
            self.item,
            self.stxm_obj.get_min_x(),
            self.stxm_obj.get_max_x(),
            self.stxm_obj.get_min_y(),
            self.stxm_obj.get_max_y(),
        )
        self.show_data(self.data, True)
        self.set_autoscale()
