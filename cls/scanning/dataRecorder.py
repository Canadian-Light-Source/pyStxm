...
from cls.scanning.nexus.nxstxm import (
    create_NXstxm_file,
    create_entry_in_NXstxm_file,
    load_single_entry_from_NXstxm_file,
    update_data_with_openclose,
    update_point_with_openclose,
    load_NXstxm_file,
    update_attrs,
    update_data,
)
...
class ThreadXSPSave(threading.Thread):
    ...
    def run(self):
        if self.data_dct != None:
            ...
            np.savetxt(
                fstr, data, delimiter="\t", fmt="%.3f"
            )  # use exponential notation
            _logger.info(
                "ThreadXSPSave: [%s] saved [%s]"
                % (self.name, os.path.join(datadir, fname))
            )
                ...
                if data is not None:
                    ...
                    if _data.ndim == 2:
                        ...
                        if self.thumb_ftype.find("tif") > -1:
                            ...
                            fname = (
                                dct_get(self.data_dct, ADO_CFG_DATA_THUMB_NAME) + ".tif"
                            )
                            fstr = os.path.join(datadir, fname)
                            ...
                            fname = (
                                dct_get(self.data_dct, ADO_CFG_DATA_THUMB_NAME) + ".jpg"
                            )
                            fstr = os.path.join(datadir, fname)
...
class NexusDataIo(QObject):
    ...
    def __init__(
        ...
        file_prefix,
        nx_def="nxstxm",
        attribs={"entry": "entry1", "counter": "counter0"},
    ):
    ...
    def write(self, dct, use_tmpfile=False, allow_tmp_rename=False):
        ...
        if use_tmpfile:
            ...
            (save_success, fname) = self.create_nexus_file(
                self.tmp_file_name, stype, data_dct=dct
            )
        else:
            ...
            if allow_tmp_rename:
                ...
                (save_success, fname) = self.create_nexus_file(
                    self.file_name, stype, data_dct=dct
                )
    ...
    def add_entry(self, entry_num, dct, use_tmpfile=False):
        ...
        if use_tmpfile:
            ...
            (save_success, fname) = self.add_nexus_entry(
                tmp_file_name, stype, data_dct=dct, entry_num=entry_num
            )
        else:
            (save_success, fname) = self.add_nexus_entry(
                self.file_name, stype, data_dct=dct, entry_num=entry_num
            )
    ...
    def add_data_point(self, entry_num, row, col, val, counter="counter0"):
        ...
        self.add_nexus_data_point(
            self.file_name, "entry%d" % entry_num, point, counter=counter
        )
...
class DataIo(QObject):
    ...
    def __init__(self, data_dir, file_prefix, options={}):
        ...
        if "standard" in list(options.keys()):
            if options["standard"] == "nexus":
                if options["def"] == "nxstxm":
                    ...
                    raise DataRecError(
                        "DataIo %s not supported NEXUS definition [%s]" % options["def"]
                    )

            elif options["standard"] == "json":
                fstr = os.path.join(self.data_dir, self.file_prefix)
                self.data_io = JSONDataIo(fstr)
                self.file_extension = ".json"
            elif options["standard"] == "csv":
                print("DataIo: csv type not supported yet")
                self.file_extension = ".csv"
            elif options["standard"] == "txt":
    ...
    def save(self, data_dct, use_tmpfile=False, allow_tmp_rename=False):
        ...
        err = None
        if self.data_io is not None:
            err = self.data_io.write(
                data_dct, use_tmpfile=use_tmpfile, allow_tmp_rename=allow_tmp_rename
            )
        return err
...
class DataRecorder(QObject):
    ...
    def save(self, dct, use_tmpfile=False, allow_tmp_rename=False):
        ...
        self.data_io.save(
            dct, use_tmpfile=use_tmpfile, allow_tmp_rename=allow_tmp_rename
        )
