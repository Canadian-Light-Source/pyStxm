...
class IOCAppsPanel(BaseContentPanel):
    ...
    def __init__(self, main_obj=None):
        ...
        for app_hrtbt_name in app_names:
            ...
            app_obj = hrt_bt_dct[app_hrtbt_name]
            app_name = hrt_bt_dct[app_hrtbt_name].desc
            vlayout.addWidget(self.make_app_sts_lbl(app_obj, app_name))
    ...
    def make_app_sts_lbl(self, dev, hdr_txt):
        ...
        app_ltgreen = master_colors["app_ltgreen"]["rgb_str"]
        app_red = master_colors["app_yellow"]["rgb_str"]
        white = master_colors["white"]["rgb_str"]
        print("IOCAppsPanel: make_app_sts_lbl: setting up [%s]" % hdr_txt)
        stsLbl = ophyd_biLabelWidget(
            dev,
            hdrText=hdr_txt,
            title_color=white,
            var_clr="blue",
