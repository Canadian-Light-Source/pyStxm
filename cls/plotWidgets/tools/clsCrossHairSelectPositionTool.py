...
class clsCrossHairSelectPositionTool(XCursorTool):
    changed = QtCore.pyqtSignal(object)
    TITLE = _("Select Center Position")
    ICON = "xcursor.png"
    CHECKABLE = True
    ...
    def __init__(
        ...
        super(clsCrossHairSelectPositionTool, self).__init__(
            manager,
            tip=_("Select Center Position"),
            #icon=icon,
    ...
    def move(self, filter, event):
        plot = filter.plot
        if not self.shape:
            # p = get_parent_who_has_attr(self, 'on_new_center_selected')
            # if p:
            #     #need a selection box before the crosshairs
            #     p.on_new_center_selected()

            self.shape = self.create_shape()
            self.shape.attach(plot)
            #self.shape.setZ(plot.get_max_z() + 1)
            self.shape.setZ(99999999999999)
