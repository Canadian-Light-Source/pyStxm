...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        self.gridparam = make.gridparam(
            background="black", minor_enabled=(False, False), major_enabled=(True, True)
        )
        self.gridparam.maj_line.color = "#515151"
        self.gridparam.min_line.color = "#515151"
        self.scanplot = CurveViewerWidget(
            winTitleStr="", options=dict(xlabel="", gridparam=self.gridparam)
        )
    ...
    def edit_params(self):
        ...
        for item in items:
            if isinstance(item, plotpy.items.GridItem):
