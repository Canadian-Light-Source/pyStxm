...
from cls.utils.json_utils import json_to_file, dict_to_json, json_to_dict
import json


def ingest_pv_dict(inp):
    type_map = {"float": float, "int": int}
    body = {}
    for (i, (k, (dtype, rec_type))) in enumerate(inp.items()):
        body[str(i)] = pvproperty(name=k, dtype=type_map[dtype], mock_record=rec_type)
