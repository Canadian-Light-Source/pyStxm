...
class curve_Obj(QtCore.QObject):
    ...
    def __init__(self, name, x=None, y=None, num_points=1, curve_style={}):
    ...
    def init_data(self, num_points):
        self.xData = np.zeros(self.max_rolling_window, dtype=float)
        self.yData = np.zeros(self.max_rolling_window, dtype=float)
    ...
    def setXYData(self, x, y):
        ...
        self.xData = np.array(x).astype("float64").flatten()
        self.yData = np.array(y).astype("float64").flatten()
            ...
            if section in dir(curve_item):
                ...
                if type(style_dct[section]) == dict:
                    for _attr in list(style_dct[section].keys()):
                        ...
                        #print("curve_object: apply_style: setattr()",sect_item, "_%s" % _attr, style_dct[section][_attr])
                        setattr(sect_item, "%s" % _attr, style_dct[section][_attr])
                        print("curve_object: apply_style: setattr()", sect_item, "%s" % _attr, style_dct[section][_attr])
                else:
                    #setattr(curve_item, "_%s" % section, style_dct[section])
                    setattr(curve_item, "%s" % section, style_dct[section])
        print()
    ...
    def initXYCurve(self, num_points):
        ...
        style_key = list(self.curve_style.keys())[0]
        self.curve_item = make.curve(
            self.xData[0:num_points],
            self.yData[0:num_points],
            linewidth=self.curve_style[style_key]['width'],
            title=self.name,
            curvestyle=self.curve_style["curvestyle"],
            color=self.curve_style[style_key]["color"],
            shade=self.curve_style["shade"],
            baseline=self.curve_style["baseline"]
        )
        #self.apply_style(self.curve_item.curveparam, self.curve_style)
        #self.curve_item.curveparam.update_curve(self.curve_item)
        ...
        #        self.curve_item.curveparam.line._color = master_colors['plot_forgrnd']["rgb_hex"]
    ...
    def add_xy_point(self, xpoint, point, update=False):
        ...
        if self.plotDataCntr >= self.max_rolling_window:
            ...
            try:
                ...
                print(f"curve_object.py: add_xy_point: An error occurred: {e}")
