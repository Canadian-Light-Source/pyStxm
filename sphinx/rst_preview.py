...
path_to_webdriver = "C:\\controls\\bin\\IEDriverServer.exe"  # This is the path to the webdriver. Change this to where you put the driver.
...
while True:
    for rst_file in rst_files:
        if isfile(rst_file):
            ...
            if (
                record[0] != new_size or record[1] < new_mtime
            ):  # Checks if file size or latest modify time change
                ...
                if (
                    record[2] == 0
                ):  # open the corresponding html for the rst file under editing
                    ...
                    content = content[
                        new_body_start_idx : content.rindex("</body>")
                    ].strip()
