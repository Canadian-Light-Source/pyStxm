...
def loadFunctionRegExps(local_lib_types_re=[]):
    ...
    fre_simple_type = [
        (re.compile("(" + _type + ")\s*([^\*\[]*)\Z"), eval(_type), 2)
        for _type in type_list
    ]
    fre_pointer_type = [
        (
            re.compile("(" + _type + ")\s*\*([^\*]*)\Z"),
            eval("POINTER(" + _type + ")"),
            2,
        )
        for _type in type_list
    ]
    fre_pointer_type_array = [
        (
            re.compile("(" + _type + ")\s*([readArray|writeArray]*)\[\]\Z"),
            array_type(_type),
            2,
        )
        for _type in type_list_array
    ]
    fre_pointer_type_2 = [
        (
            re.compile("(" + _type + ")\s*([^\s]*)\[\]\Z"),
            eval("POINTER(" + _type + ")"),
            2,
        )
        for _type in type_list
    ...
    fre_char_etoile = [
        (re.compile(r"(char)\s*\*([^\*]*)\Z"), c_char_p, 2)
    ]  # match "char * name"
    fre_void_etoile = [
        (re.compile(r"(void)\s*\*([^\*]*)\Z"), c_void_p, 2)
    ]  # match "void * name"
    fre_void_etoile.append(
        (re.compile(r"(void)\s*\**([^\*]*)\Z"), c_void_p, 2)
    )  # match "void ** name"
    # fre_void_etoile.append( (re.compile(r'(HANDLE)\s*\**([^\*]*)\Z'), c_void_p, 2) ) # match "HANDLE name"
    fre_char_array = [
        (re.compile(r"(char)\s*([^\s]*)\[\]"), c_char_p, 2)
    ]  # match "char name[]"
