...
from cls.applications.pyStxm.widgets.scan_table_view.spatialTableView import (
...
class SpatialSelWidget(BaseSelectionWidget):
    ...
    def __init__(
        ...
        self.table_view = SpatialScanTableView(
            use_center=use_center, is_point=is_point, is_arb_line=is_arb_line
        )
    ...
    def set_spatial_range(self, qrect):
        ...
        self.table_view.set_x_spatial_range(
            abs(qrect.width())
        )  # (xleft, ytop, width, height)
        self.table_view.set_y_spatial_range(
            abs(qrect.height())
        )  # (xleft, ytop, width, height)
    ...
    def set_spatial_startstop(self, qrect):
        ...
        self.table_view.set_x_spatial_startstop(
            (x1, x2)
        )  # (xleft, ytop, width, height)
    ...
    def check_e712params(self, scan):
        ...
        for e_roi in scan[EV_ROIS]:
            ...
            if total_pnts_needed > max_ttl_points:
                _logger.error(
                    "INVALID SCAN: max number of temperal points required for scan has exceeded limit available on E712 controller"
                )
                _logger.error(
                    "Try reducing either the number of points for X or dwell time per point"
                )
                notify(
    ...
    def on_new_region(
        self, scan=None, add_energy_roi=True, ev_only=False, x_cntr=None, y_cntr=None
    ):
        """
        ...
        if scan is None:
            ...
            if (cur_scan is None) and (len(slist) == 0):
                ...
                x_roi = get_base_roi(
                    SPDB_X,
                    self.xpos_name,
                    cur_scan[SPDB_X][CENTER],
                    cur_scan[SPDB_X][RANGE],
                    cur_scan[SPDB_X][NPOINTS],
                    stepSize=None,
                    max_scan_range=None,
                    enable=True,
                )
                y_roi = get_base_roi(
                    SPDB_Y,
                    self.ypos_name,
                    cur_scan[SPDB_Y][CENTER],
                    cur_scan[SPDB_Y][RANGE],
                    cur_scan[SPDB_Y][NPOINTS],
                    stepSize=None,
                    max_scan_range=None,
                    enable=True,
                )
    ...
    def load_scan(self, loaded_scan=None, ev_only=False, sp_only=False):
        ...
        if SPDB_EV_ID in loaded_scan.keys():
            model_id = loaded_scan[SPDB_EV_ID]
        else:
            model_id = 0
            loaded_scan[SPDB_EV_ID] = 0

        self.ev_sel_widget.table_view.create_new_model(model_id=model_id)
    ...
    def add_scan(self, loaded_scan=None, ev_only=False, sp_only=False):
        ...
        self.scan_id = loaded_scan[SPDB_ID_VAL]
        if SPDB_EV_ID in loaded_scan.keys():
            ev_id = loaded_scan[SPDB_EV_ID]
        else:
            ev_id = 0

        if not sp_only:
            # create a new ev roi model with the scan[SPDB_EV_ID] as its key
            self.ev_sel_widget.table_view.create_new_model(
                model_id=ev_id
            )
            self.ev_sel_widget.load_scan(loaded_scan[SPDB_EV_ROIS])
