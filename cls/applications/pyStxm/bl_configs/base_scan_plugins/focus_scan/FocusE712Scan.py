...
class BaseFocusE712ScanClass(BaseScan):
    """
    ...
    def init_subscriptions(self, ew, func):
        ...
        if self.is_pxp:
            ...
            # self._emitter_cb = ImageDataEmitter(DNM_DEFAULT_COUNTER, y=DNM_ZONEPLATE_Z,
            #                                     x=DNM_SAMPLE_X,
    ...
    def make_pxp_scan_plan(self, dets, md=None, bi_dir=False):
        ...
        def do_scan():
            ...
            if self.is_zp_scan:
                ...
                yield from bps.mv(
                    finemtrx, self.zx_roi[CENTER], finemtry, self.zy_roi[CENTER]
                )
                ...
                yield from bps.mv(
                    finemtrx, self.x_roi[CENTER], finemtry, self.y_roi[CENTER]
                )
                ############################
    ...
    def make_lxl_scan_plan(self, dets, md=None, bi_dir=False):
        ...
        def do_scan():
            ...
            if self.is_zp_scan:
                # moving them to the start gets rid of a goofy first line of the scan
                yield from bps.mv(
                    finemtrx, self.zx_roi[START], finemtry, self.zy_roi[START]
                )
                ...
                # moving them to the start gets rid of a goofy first line of the scan
                yield from bps.mv(
                    finemtrx, self.x_roi[START], finemtry, self.y_roi[START]
                )
                ############################
    ...
    def configure(self, wdg_com, sp_id=0, line=False):
        ...
        self.seq_map_dct = self.generate_2d_seq_image_map(
            1, self.zz_roi[NPOINTS], self.x_roi[NPOINTS], lxl=self.is_lxl
        )
    ...
    def modify_config_for_hdw_accel(self):
        ...
        if self.is_pxp:
            ...
            # APR 21 2022 ddl_tbl_pv = MAIN_OBJ.device('e712_ddl_tbls')
