...
def modify_2posner_ctrl_data_grps(parent, nxgrp, doc, scan_type):
    ...
    if len(prim_data_lst) < ttlpnts:
        ...
        xdata = np.array(
            parent._data["primary"][x_src][uid]["data"][0:xnpoints], dtype=np.float32
        )
        # ydata is every ynpoint
        ydata = np.array(
            parent._data["primary"][y_src][uid]["data"][0::ynpoints], dtype=np.float32
        )
    ...
    sr_data = np.array(
        parent._data["primary"][parent.get_devname(DNM_RING_CURRENT) + "_val"][uid][
            "data"
        ],
        dtype=np.float32,
    )
...
def modify_base_2d_nxdata_group(parent, data_nxgrp, doc, scan_type):
    ...
    if scan_types(scan_type) in three_d_scans:
        ...
        det_data = np.array(
            parent._data["primary"][det_nm][uid]["data"], dtype=np.float32
        )
        if resize_data:
...
def modify_base_2d_instrument_group(parent, inst_nxgrp, doc, scan_type):
    ...
    det_data = np.array(
        parent._data["primary"][det_nm][uid]["data"]
