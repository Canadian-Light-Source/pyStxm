...
class BaseFineImageScansParam(MultiRegionScanParamBase):
    def __init__(
        ...
        if self.sample_positioning_mode == sample_positioning_modes.GONIOMETER:
            ...
            x_cntr = self.main_obj.device("DNM_COARSE_X").get_position()
            y_cntr = self.main_obj.device("DNM_COARSE_Y").get_position()
        ...
        self.multi_region_widget.ev_total_changed.connect(self.update_est_time)
        ...
        if hasattr(self, 'scanTypeSelComboBox'):
        ...
        if hasattr(self, 'hdwAccelDetailsBtn'):
    ...
    def gen_max_scan_range_limit_def(self):
        ...
        if self.sample_positioning_mode == sample_positioning_modes.GONIOMETER:
            ...
            mtr_sx = self.main_obj.device("DNM_COARSE_X")
            mtr_sy = self.main_obj.device("DNM_COARSE_Y")
            ...
            bounding = ROILimitObj(
                bounding_qrect,
                get_alarm_clr(255),
                "Range is beyond CoarseXY Capabilities",
                get_warn_fill_pattern(),
            )
            normal = ROILimitObj(
                bounding_qrect,
    ...
    def mod_roi(self, wdg_com, do_recalc=True, ev_only=False, sp_only=False):
        ...
        #    return
            ...
            # we are
            print(
                "image_scan: mod_roi: unsupported widget_com CMND [%d]" % wdg_com[CMND]
            )
            return
    ...
    def load_roi(self, wdg_com, append=False, ev_only=False, sp_only=False):
        ...
        has_scan_subtype_selector = False
        if hasattr(self,'scanTypeSelComboBox'):
            has_scan_subtype_selector = True
    ...
    def update_type(self):
        ...
        if self.wdg_com is not None:
            ...
            for sp_id in sp_ids:
                ...
                e_setpoints =self.multi_region_widget.get_just_ev_setpoints()
                if (len(sp_ids) > 1) or (not self.multi_region_widget.is_set_for_single_ev() and len(e_setpoints) > 1):
                    self.type = scan_types.SAMPLE_IMAGE_STACK
                    dct_put(
                        sp_db, SPDB_SCAN_PLUGIN_TYPE, scan_types.SAMPLE_IMAGE_STACK
    ...
    def update_data(self, force_pxp=False):
        """
