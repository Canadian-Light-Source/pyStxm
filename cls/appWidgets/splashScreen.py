...
class CSplashScreen(QtWidgets.QFrame):
    ...
    def showMessage(
        self, theMessage, theAlignment=QtCore.Qt.AlignLeft, theColor=QtCore.Qt.black
    ):
...
def show_splash(path):
    ...
    splashScreen.show()
    # RUSS FEB 24 QtWidgets.QApplication.processEvents()
    for count in range(1, 6):
        splashScreen.showMessage(
            splashScreen.tr("Processing %1...").arg(count),
...
class SplashScreen(QSplashScreen):
    def __init__(self, img_path=splashPath, ver_str="Version 0.0"):
        ...
        # self.hide()
        # RUSS FEB 24 QtWidgets.QApplication.processEvents(QtCore.QEventLoop.AllEvents)
...
def create_splash(img_path=splashPath, ver_str="Version 0.0"):
    global _splash_screen
    _splash_screen = SplashScreen(img_path=img_path, ver_str=ver_str)
    return _splash_screen
...
def get_splash(img_path=splashPath, ver_str="Version 0.0"):
    ...
    all_widgets = curr_app.allWidgets()
    for w in all_widgets:
        if type(w) == SplashScreen:
            return w
