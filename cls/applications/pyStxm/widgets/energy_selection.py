...
from cls.utils.fileUtils import (
    loadDatToXY,
    loadDatToArray,
    loadDatTo2DArray,
    readColumnStrs,
...
class EnergySelector(CurveViewerWidget):
    ...
    def __init__(
        self, winTitleStr="Energy Selector", toolbar=False, show_table=False, options={}
    ):
        super(EnergySelector, self).__init__(
            winTitleStr=winTitleStr, toolbar=toolbar, options={}
        )
        ...
        icon4.addPixmap(
            QtGui.QPixmap(_fromUtf8(":/guiqwt/icons/images/xrange.png")),
            QtGui.QIcon.Normal,
            QtGui.QIcon.Off,
        )
        icon4.addPixmap(
        ...
        spacer = QtWidgets.QSpacerItem(
            5000, 100, QtWidgets.QSizePolicy.Maximum, QtWidgets.QSizePolicy.Expanding
        )
    ...
    def create_new_range(self, center=None, rng=None, dosignal=True):
        ...
        if dosignal:
            self.new_range.emit(
                self.energy_id, self._rng_center, self._rng_total, self._rng_npoints
            )
        self.update_curve()
        return self.energy_id
    ...
    def range_changed(self, regionWidget, rng):
        ...
        print(
            "range changed: %s: %f, %f"
            % (self.energy_id, self._rng_center, self._rng_total)
        )
        self.update_edit_widgets()
        # self.mapper.submit()
        self.rng_changed.emit(
            self.energy_id, self._rng_center, self._rng_total, self._rng_npoints
        )
    ...
    def mouseReleaseEvent(self, ev):
        ...
        if ev.button() == QtCore.QtCore.Qt.MouseButton.MiddleButton:
            self.zoom_to_range()
