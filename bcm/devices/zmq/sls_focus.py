...
CONFIG = {'REQ_adress': 'tcp://<PUB host name here>:56562',  # ZMQ Request port
          'SUB_adress': 'tcp://<PUB host name here>:56561',  # ZMQ subscription port
...
class App(QtWidgets.QMainWindow):
    ...
    def handle_scanStarted(self, scan_request):
        ...
        if scan_request['scanType'] in ['Focus', 'OSA Focus']:
            ...
            points_dict = {x["trajectories"][0]["positionerName"]: x["nPoints"] for x in
                           scan_request["innerRegions"][0]["axes"]}
            extent_dict = {}
            ...
            self.scan_extent = [extent_dict["Zoneplate"][0], extent_dict["Zoneplate"][1], extent_dict["FineX"][0],
                                extent_dict["FineX"][1]]
            self.scan_shape = [0, 0, points_dict["Zoneplate"], points_dict["FineX"]]
            self.scan_data = numpy.full(self.scan_shape[2:], numpy.nan)
            self.scan_axis_titles = ["Zoneplate", "FineX"]
            self.canvas.clear()
            self.canvas.plot()
            self.userFocusButtons(enable=False)
    ...
    def handle_scanLineData(self, data):
        ...
        self.scan_data[offset[0] + chunk_start[0]:offset[0] + chunk_start[0] + chunk_shape[0],
        offset[1] + chunk_start[1]:offset[1] + chunk_start[1] + chunk_shape[1]] = chunk_values
