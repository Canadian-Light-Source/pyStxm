...
def load_beamline_device_config(bl_config_nm):
    ...
    """
    cfgDir = os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "../applications/pyStxm/bl_configs", bl_config_nm
    )
    if not os.path.exists(cfgDir):
        _logger.error(
            "Beamline device Configuration directory [%s] does not exist" % cfgDir
        )
        exit()
...
def load_beamline_preset(bl_config_nm):
    ...
    if not os.path.exists(cfgDir):
        _logger.error(
            "Beamline preset Configuration directory [%s] does not exist" % cfgDir
        )
        exit()
