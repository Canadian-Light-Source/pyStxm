...
def get_base_roi(
    ...
    if rng == None:
        ...
        roi_def[START] = center - (0.5 * rng)
        roi_def[STOP] = center + (0.5 * rng)
    ...
    # added to allow non default behaviour when recalculating the values of an roi,
    # ex: force on_range_changed() to keep the current START value and adjust CENTER and STOP,
    # default is that CENTER would would be used to determine new START and END
...
def determine_setpoints(roi_def, stepSize=None):
    if roi_def[NPOINTS] == 1:
        roi_def[SETPOINTS] = np.linspace(
            roi_def[START] + roi_def[OFFSET],
            roi_def[STOP] + roi_def[OFFSET],
            int(roi_def[NPOINTS]),
            endpoint=True,
            retstep=False,
        )
        ...
        if stepSize:
            ...
            roi_def[SETPOINTS] = np.arange(
                roi_def[START] + roi_def[OFFSET],
                roi_def[STOP] + roi_def[OFFSET] + stepSize,
                stepSize,
            )
        else:
            roi_def[SETPOINTS], roi_def[ROI_STEP] = np.linspace(
                roi_def[START] + roi_def[OFFSET],
                roi_def[STOP] + roi_def[OFFSET],
                int(roi_def[NPOINTS]),
                endpoint=True,
...
def set_use_center(roi):
    roi[USE_CENTER] = True  # default
    roi[USE_START] = False

def set_use_start(roi):
    roi[USE_CENTER] = False
    roi[USE_START] = True # override default
...
def on_fixed_start_changed(roi):
    """
    here there is a new start value that we want fixed and recalc everything else
    """
    check_if_single_point(roi, True)
    recalc_stop(roi, use_start=True)
    recalc_center(roi)
    recalc_setpoints(roi)



def on_ev_start_changed(roi):
    """
    use as is: START, RANGE
    recalc: STOP, CENTER, SETPOINTS
    no change: STEP_SIZE, NPOINTS
    """
    check_if_single_point(roi, True)
    if not roi[IS_POINT]:
        if USE_START in roi.keys():
            use_start = roi[USE_START]
        else:
            use_start = False
        recalc_stop(roi, use_start)
    recalc_center(roi)
    recalc_range(roi)
...
def on_range_changed(roi, use_start=False):
    check_if_single_point(roi, False)
    if USE_START in roi.keys():
        use_start = roi[USE_START]
    else:
...
def set_ev_single_point(roi):
    """
    This function will force the ev roi to be a single point using the value in START as the point value
    Args:
        roi:
    ...
    """
    roi[NPOINTS] = 1
    on_ev_start_changed(roi)
    roi[EV_BOUNDARY_ROI_START] = roi[START]
    return roi
...
def recalc_stop(roi, use_start=False):
    if roi[IS_POINT]:
        roi[STOP] = roi[CENTER]
...
def recalc_setpoints(roi):
    ...
    if roi[NPOINTS] == 1:
        ...
        if roi[ROI_STEP] != 0.0:
            roi[SETPOINTS], STEP = np.linspace(
                roi[START], roi[STOP], int(roi[NPOINTS]), endpoint=True, retstep=True
            )
        else:
            roi[SETPOINTS] = np.linspace(
                roi[START], roi[STOP], int(roi[NPOINTS]), endpoint=True, retstep=False
            )
    else:
        ...
        roi[SETPOINTS], roi[ROI_STEP] = np.linspace(
            roi[START], roi[STOP], int(roi[NPOINTS]), endpoint=True, retstep=True
        )
    roi["RECT"] = (
        roi[START],
...
def get_base_energy_roi(
    ...
    stepSize=None,
    enable=False,
    energy_scan_order=energy_scan_order_types.EV_THEN_POL,
):
    """define a base Region of Interest def that is passed around
    'EV''start':
    'EV''stop':
    ...
    """
    roi_def = get_base_start_stop_roi(
        name, positionerName, start, stop, npoints, stepSize
    )
    roi_def[ID] = EV_ROI
    roi_def[ID_VAL] = -1
...
def make_spatial_db_dict(
    ...
    dct_put(
        dct, WDGCOM_CMND, None
    )  # The caller sets this field to a widget_com_cmnd_type
    ...
    dct_put(
        dct, SPDB_RECT, (x_roi[START], y_roi[START], x_roi[STOP], y_roi[STOP])
    )  # holds total number of EV points
    ...
    dct_put(
        dct, SPDB_PLOT_IMAGE_TYPE, None
    )  # used to help determine what to plot when a scan is loaded. image_types-> Enum('focus', 'osafocus','image', 'line_plot')
    dct_put(
        dct, SPDB_PLOT_SHAPE_TYPE, None
    )  # the plot shape item type: spatial_type_prefix-> ROI, SEG, PNT

    dct_put(
        dct, SPDB_SCAN_PLUGIN_TYPE, None
    )  # the scan type: scan_types-> Enum('Detector_Image','OSA_Image','OSA_Focus','Sample_Focus','Sample_Point_Spectrum', 'Sample_Line_Spectrum', 'Sample_Image', 'Sample_Image_Stack', 'Generic_Scan')
    dct_put(
        dct, SPDB_SCAN_PLUGIN_SUBTYPE, None
    )  # the scan sub type: scan_sub_types = Enum('Point_by_Point', 'Line_Unidir')
    # dct_put(dct, SPDB_SCAN_PLUGIN_ITEM_ID, None)		#the id of the currently selected scan, set in a scan pluggin widget
    dct_put(
        dct, SPDB_SCAN_PLUGIN_SECTION_ID, None
    )  # a string saying with the scan section ID is, used for determining
    dct_put(
        dct, SPDB_SCAN_PLUGIN_DATAFILE_PFX, None
    )  # a one character prefix for the scan type, using this to save files with a
    dct_put(
        dct, SPDB_SCAN_PLUGIN_MAX_SCANRANGE, (None, None)
    )  # scans like zp_image_scan have a limited range, so have pluggin define the range so that plot etc can make use of this info
    dct_put(
        dct, SPDB_SPATIAL_ROIS_CENTER, (None, None)
    )  # if there is a multi spatial roi scan this will contain the center of all of those scans
    dct_put(
...
def get_scan_type_str_from_wdg_com(wdg_com):
    """
    from the wdg_com return the scan type string
    Parameters
    ----------
    wdg_com
    ...
    scan_type_str = dct_get(sp_db, SPDB_SCAN_PLUGIN_SECTION_ID)
    return scan_type_str



def get_dwell_from_wdg_com(wdg_com):
    """
    return dwell from first ev region
    Parameters
    ----------
    wdg_com
    ...
    """
    sp_db = get_first_sp_db_from_wdg_com(wdg_com)
    ev_rois = dct_get(sp_db, SPDB_EV_ROIS)
    dwell_ms = ev_rois[0][DWELL]
    return dwell_ms

def get_ev_rois_from_wdg_com(wdg_com):
    """
    return all ev regions
    Parameters
    ----------
    wdg_com
...
def get_total_ev_pol_npoints_from_ev_rois(ev_rois):
    """
    walk the list of energy rois and total up the number of points which is typically the number of images that
    will be needed
    Parameters
    ----------
    ev_rois
    ...
    for e_roi in ev_rois:
        npoints_ev += e_roi[NPOINTS]
        for pol_roi in e_roi[POL_ROIS]:
            npoints_pol += 1

    total = npoints_ev * npoints_pol
    return total


def get_axis_roi_from_wdg_com(axis_str, wdg_com):
    """
    return dwell from first ev region
    Parameters
    ----------
    ...
    """
    sp_db = get_first_sp_db_from_wdg_com(wdg_com)
    if axis_str in sp_db.keys():
        axis_dct = sp_db[axis_str]
    else:
        axis_dct = {}
    return axis_dct
...
def get_next_unique_id(regions: Optional[List[dict]], id_key: Optional[str] = None) -> int:
    if regions:
        # the following assumes either *all* ROIs have the id_key, or *none* of them do
        existing_ids = [
            int(region.get(id_key, index))
            for index, region in enumerate(regions)
        ]
        if existing_ids:
            return min(set(range(max(existing_ids) + 2)) - set(existing_ids))
