...
class BaseModel(QtCore.QAbstractTableModel):
    ...
    def get_model_status_data(self):
    ...
    def set_data_status(self, row, val):
        """
        set the status data for the specified row
        @param row:
        @param val:
        @return:
        """
        # print(f"scan_queue_table: set_data_status: row={row} val={val}")
        try:
            self.model_status[row] = val
        except Exception as e:
            # Catch all exceptions
            msg = f"scan_queue_table: set_data_status: An error occurred: {e}"
            _logger.error(msg)
            print(msg)
...
class ScanQTableModel(QtCore.QAbstractTableModel):
    def __init__(self, parent, model_d, header, *args):
        ...
        #self.model_status_data = scan_q_model_class.model_status
    ...
    def set_header(self, hdr_list):
        ...
        self.headerDataChanged.emit(QtCore.Qt.Horizontal, 0, len(self.header) - 1)
    ...
    def insertRow(self, item, parent=QtCore.QModelIndex()):
        ...
        #self.model_status_data.append(scan_status_types.STOPPED)
        scan_q_model_class.model_status.append(scan_status_types.STOPPED)
        self.endInsertRows()
    ...
    def removeAll(self, parent=QtCore.QModelIndex()):
        ...
        if self.rowCount() > 0:
            ...
            self.endRemoveRows()
        scan_q_model_class.model_status = []
        self.dataChanged.emit(parent, parent)
        self.model_data = self.scan_q_model_class.get_model_data()
    ...
    def data(self, index, role=QtCore.Qt.DisplayRole):
        ...
        sts = scan_q_model_class.model_status[row]
        ...
        if (col == 0) and (role == QtCore.Qt.TextAlignmentRole):
            ...
            if sts == scan_status_types.STOPPED:
                bg = master_colors["app_medgray"]["rgb_qclr"]
            elif sts == scan_status_types.PAUSED:
                bg = master_colors["app_yellow"]["rgb_qclr"]
            elif sts == scan_status_types.RUNNING:
                bg = master_colors["scan_sts_blue"]["rgb_qclr"]
            elif sts == scan_status_types.ABORTED:
                bg = master_colors["app_red"]["rgb_qclr"]
            else:
                bg = master_colors["app_drkgray"]["rgb_qclr"]

            return bg

        elif role == QtCore.Qt.ForegroundRole:
            if sts == scan_status_types.STOPPED:
                fg = master_colors["app_superltgray"]["rgb_qclr"]
            elif sts == scan_status_types.PAUSED:
                ...
                fg = master_colors["app_blue"]["rgb_qclr"]
            else:
                #sts = str(sts)
                #_logger.error(f"unknown scan status [{sts}]")
                fg = None
            return fg
    ...
    def headerData(self, col, orientation, role=QtCore.Qt.DisplayRole):
        # print(f"headerData: header=[{self.header}] col={col}")
...
class ScanQTableView(QtWidgets.QTableView):
    ...
    def adjust_dock_width(self):
        # Automatically resize the dock widget to the width of its contents
        self.dock_widget.adjustSize()

        # Optionally add a margin if desired, or further adjust size
        contents_width = self.dock_content.document().idealWidth() + self.dock_widget.contentsMargins().left() + self.dock_widget.contentsMargins().right()
        self.dock_widget.setFixedWidth(contents_width)
...
class ProgressBarDelegate(QtWidgets.QItemDelegate):
    ...
    def setEditorData(self, editor, index):
        ...
        # print(f"setEditorData: data={data:.2f}")
        if data == 0.0:
            editor.setStyleSheet(
                "*::chunk{ background-color: %s }" % master_colors["app_drkgray"]["rgb_str"]
            )
            editor.setStyleSheet("*{ color: %s }" % master_colors["white"]["rgb_str"])
        elif data < 100.0:
            editor.setStyleSheet(
                "*::chunk{ background-color: %s }" % master_colors["scan_sts_blue"]["rgb_str"]
            )
            editor.setStyleSheet("*{ color: %s }" % master_colors["white"]["rgb_str"])
        else:
            editor.setStyleSheet(
...
class ScanQueueTableWidget(QtWidgets.QWidget):
    ...
    def __init__(self, kwargs={}, parent=None):
        ...
        table_view = ScanQTableView(
            header, prog_col=DEFAULT_PROGESS_BAR_COLUMN, pmap_col=DEFAULT_PIXMAP_COLUMN
        )
        ...
        # self.table_view.hideColumn(DEFAULT_PIXMAP_COLUMN)
    ...
    def scroll_to_row_with_spid(self, sp_id):
        if sp_id in list(self.spid_to_row_dct.keys()):
            ...
            # hack
            if len(list(self.spid_to_row_dct.keys())) == 1:
                self.scroll_to_row(0)
                return 0
            else:
                _logger.error(
                    "scroll_to_row_with_spid: sp_id[%d] does not exist in table" % sp_id
                )
            ...
            for i in range(h.count()):
                if not h.isSectionHidden(i) and h.sectionViewportPosition(i) >= 0:
                    self.table_view.scrollTo(
                        self.table_view.model().index(row, i),
                        self.table_view.PositionAtCenter,
                    )
    ...
    def set_pixmap(self, row, val):
        ...
        scan_q_model_class.set_data_status(row, val)
        # always change focus off of widget
        # self.dirLabel.setFocus()
    ...
    def resize_tableview(self):
        ...
        """
        orig_size = self.table_view.size()  # New size after "resize"
        new_size = orig_size
        new_size.setWidth(orig_size.width() + 1)
        event = QtGui.QResizeEvent(new_size, orig_size)
        self.table_view.resizeEvent(event)
    ...
    def set_delegates(self, scan_type):
        ...
        elif scan_type == scan_types.PATTERN_GEN:
            self.set_delegates_ev()

        elif scan_type == scan_types.TWO_VARIABLE_IMAGE:
            self.set_delegates_ev()
    ...
    def set_delegates_ev(self):
        ...
        #print(f"set_delegates_ev: setting header to [{tm.header}]")
    ...
    def set_delegates_multi_ev(self):
        ...
        """
        tm = self.table_view.model()
        #print(f"set_delegates_multi_ev: setting header to [{tm.header}]")
        for row in range(0, tm.rowCount()):
            self.table_view.openPersistentEditor(tm.index(row, self.prog_col))
            #self.table_view.openPersistentEditor(tm.index(row, self.pmap_col))
            # self.table_view.setRowHeight(row, 15)
    ...
    def set_delegates_point_spec(self):
        ...
        """
        tm = self.table_view.model()
        #print(f"set_delegates_point_spec: setting header to [{tm.header}] pmap_col={self.pmap_col} prog_col={self.prog_col}")
    ...
    def set_delegates_line_spec(self):
        ...
        # ['A1-00001.hdf5', 383.0, 'Circ Left', '50.7,46.4', '100x100', 11, 0],
        self.table_view.setColumnWidth(0, FILENAME_WIDTH)
        self.table_view.setColumnWidth(1, 2 * EV_WIDTH)
        self.table_view.setColumnWidth(2, EV_WIDTH)
    ...
    def make_point_scan_row_entries(self, sp_db, sample_pos=1):
        ...
        # must set these before setting the header
    ...
    def make_multi_ev_row_entries(
        self, sp_db, sample_pos=1, is_point=False, stack_dir=False
    ):
        """
        Take an single ev sp_db (detector, osa) and create a row entry for a detector scan
        The results should be a single row entry:
        ...
        # z_roi = dct_get(sp_db, SPDB_Z)
        e_rois = dct_get(sp_db, SPDB_EV_ROIS)
        ev_setpoints = self.get_all_ev_setpoints(e_rois)
        pol_setpoints = e_rois[0]['EPU_POL_PNTS']
        row_num = 0
    ...
    def get_all_ev_setpoints(self, e_rois)->list:
        """
        walk the list of energy dicts and pull out just the setpoints and return as a list
        """
        ev_setpoints = []
        for eroi in e_rois:
            ev_setpoints.append(eroi[SETPOINTS])

        ev_setpoints = merge_to_one_list(ev_setpoints)
        ev_setpoints.sort()
        return ev_setpoints

    def make_multi_ev_with_theta_row_entries(
        self, sp_db, sample_pos=1, stack_dir=False
    ):
        """
        Take an single ev sp_db (detector, osa) and create a row entry for a detector scan
        The results should be a single row entry:
            ...
            for e_roi in e_rois:
                for pol in e_roi[EPU_POL_PNTS]:
                    for e in e_roi[SETPOINTS]:
                        ...
                        # lst.append('%.2f x %.2f um' % (x_roi[RANGE], y_roi[RANGE]))
                        lst.append(
                            "(%.2f, %.2f, %.2f" % (x_roi[CENTER], y_roi[CENTER], gt)
                        )
                        # if(gt_roi):
