...
from cls.types.stxmTypes import scan_types, acceptable_2dimages_list
from cls.utils.roi_utils import get_first_sp_db_from_wdg_com

fnames = [
    "S:/STXM-data/Cryo-STXM/2017/guest/0922/C170922000.hdf5",
...
class ThreadpoolImageLoader(QWidget):
    ...
    def get_image_data(
        self, fnames, addimages=True, counter=None, progress_callback=None
    ):
        """
        ...
        for fname in fnames:
            ...
            scan_type = entry_dct[ekey]["WDG_COM"]["SPATIAL_ROIS"][sp_id[0]][
                "SCAN_PLUGIN"
            ]["SCAN_TYPE"]
            ...
            if not (scan_type in acceptable_2dimages_list) and (num_files > 1):
                continue

            counter = data_io.get_default_detector_from_entry(entry_dct)
            ...
            if data.ndim != 2:
                ...
                print(
                    "Data in file [%s] is of wrong dimension, is [%d] should be [2]"
                    % (fname, data.ndim)
                )
            else:
                ...
                item = make.image(
                    data, interpolation="nearest", colormap="gist_gray", title=None
                )
                # item.set_selectable(True)
                (x1, y1, x2, y2) = dct_get(sp_db, SPDB_RECT)
                #self.set_image_parameters(item, x1, y1, x2, y2, title=fprefix[-3:])
                self.set_image_parameters(item, x1, y1, x2, y2, title=fprefix)
    ...
    def load_image_items(
        self, fnames, result_fn, progress_fn=None, thread_complete_fn=None
    ):
...
if __name__ == "__main__":
    ...
    window.load_image_items(
        fnames,
        progress_fn=progress_fn,
        result_fn=result_output,
        thread_complete_fn=thread_complete,
    )
    progbar.show()
    app.exec_()
