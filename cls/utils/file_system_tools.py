...
def skip_non_standard_dirs(dirs, prefix_char=None):
    """
    this function used to accept a single prefix character but in order to add support for other DCS servers and diff
    bl configurations this func has been modified to accept any alphabetical character as a prefix
    """
    dirs_with_prefix = []
    # Use string.ascii_uppercase for all uppercase alphabet letters
    prefix_chars = string.ascii_uppercase
    f = lambda x: (x[0] in prefix_chars)
...
def get_next_dir_in_seq_num(path, prefix_char="C", extension="hdf5"):
    ...
    if os.path.isdir(path):
        ...
        if len(_dirs) > 0:
            #remove the prefix char whatever it is
            seq_num_str = _dirs[-1][1:]
...
def check_if_tmp_or_final_exist(path, file_num, prefix_char="C", extension="hdf5"):
    ...
    final_fname = os.path.join(
        path, "%c%d.%s.final" % (prefix_char, file_num, extension)
    )
    ...
    if os.path.isdir(data_dir):
        ...
        if num_desired_datafiles > 1:
            ...
            if stack_dir:
                ...
                for j in range(num, num + int(num_desired_datafiles)):
                    n_dct[i] = dct.copy()
                    n_dct[i]["thumb_name"] = "%s_%03d" % (
                        (prefix_char + str(data_file_seq_num)),
                        j,
                    )
                    n_dct[i]["stack_flbl"] = "%s.%s img/%d" % (
                        (prefix_char + str(data_file_seq_num)),
                        dat_ext,
                        j+1,
                    )
                ...
                next_seq_num += 1
                for j in range(
                    next_seq_num, next_seq_num + int(num_desired_datafiles - 1)
                ):
                    n_dct[i] = dct.copy()
