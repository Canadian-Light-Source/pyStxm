...
class BaseLineScanParam(MultiRegionScanParamBase):
    def __init__(
        ...
        uic.loadUi(os.path.join(ui_path, "line_scan.ui"), self)
    ...
    def on_plugin_focus(self):
        ...
        """
        if self.isEnabled():
            if not self.is_line_spec_image():
    ...
    def mod_roi(self, sp_db, do_recalc=True, ev_only=False, sp_only=False):
        ...
        if sp_db[CMND] == widget_com_cmnd_types.ROI_CHANGED:
            ...
            if cur_scan is None:
                ...
                #force Y to use same number of points as X
                npoints = cur_scan[SPDB_X][NPOINTS]
                ...
                # if its a horizontal line then set accordingly
                if scan[SPDB_Y][START] == scan[SPDB_Y][STOP]:
                    cur_scan[SPDB_Y][START] = scan[SPDB_Y][START]
                    cur_scan[SPDB_Y][STOP] = scan[SPDB_Y][STOP]
                    cur_scan[SPDB_Y][RANGE] = scan[SPDB_Y][RANGE]
                    cur_scan[SPDB_Y][STEP] = scan[SPDB_Y][STEP]
                    cur_scan[SPDB_Y][NPOINTS] = npoints
                    on_npoints_changed(cur_scan[SPDB_Y])
            ...
            _logger.error(
                "line_spec_scan: mod_roi: unsupported widget_com CMND [%d]"
                % sp_db[CMND]
            )
            return
    ...
    def load_roi(self, wdg_com, append=False, ev_only=False, sp_only=False):
        ...
        if wdg_com[CMND] == widget_com_cmnd_types.LOAD_SCAN:
            ...
            if not ev_only and (
                dct_get(sp_db, SPDB_SCAN_PLUGIN_TYPE) != scan_types.SAMPLE_LINE_SPECTRUM
            ):
                return
    ...
    def update_data(self, force_pxp=False):
        ...
        :param: force_pxp  this was added so that ingheriting plugins for bl configs that have stages
        that do not support constant velocity would force the scan sub_type to be PxP
        ...
        if None in [y_roi[START], y_roi[STOP]]:
            _logger.error("y_roi has Nones")

        if y_roi:
            # self.sub_type_override = self.scanTypeSelComboBox.currentIndex()
            if abs(y_roi[STOP] - y_roi[START]) < 0.5:
    ...
    def add_goni_rois(self, wdg_com):
        ...
        gx_roi = get_base_start_stop_roi(
            SPDB_GX, "DNM_GONI_X", gx_roi[START], gx_roi[STOP], nx
        )
        gy_roi = get_base_start_stop_roi(
            SPDB_GY, "DNM_GONI_Y", gy_roi[START], gy_roi[STOP], nx
        )
    ...
    def create_sub_spatial_dbs(self, wdg_com):
        ...
        for sp_id in sp_dbs:
            ...
            scan_rect = QtCore.QRectF(
                QtCore.QPointF(rect[0], rect[1]), QtCore.QPointF(rect[2], rect[3])
            )
            dx = scan_rect.center().x() - mtr_gx.get_position()
            dy = scan_rect.center().y() - mtr_gy.get_position()
