...
from cls.applications.pyStxm.widgets.scan_table_view.arbitraryLineTableModel import (
    ArbitraryLineScanTableModel,
)
from cls.applications.pyStxm.widgets.scan_table_view.baseScanTable import *
from cls.applications.pyStxm.widgets.scan_table_view.spatialTableModel import (
    SpatialScanTableModel,
)
...
class SpatialScanTableView(BaseScanTableView):
    def __init__(
        ...
        if is_arb_line:
            super(SpatialScanTableView, self).__init__(
                self.hdrList, scanList, ArbitraryLineScanTableModel, parent
            )
        else:
            super(SpatialScanTableView, self).__init__(
                self.hdrList, scanList, SpatialScanTableModel, parent
            )
    ...
    def init_model(self, use_center=True, is_arb_line=False):
        ...
        """
        if is_arb_line:
            self.tablemodel = ArbitraryLineScanTableModel(
                self.hdrList, self.scans, self, use_center=use_center
            )
        else:
            self.tablemodel = SpatialScanTableModel(
                self.hdrList, self.scans, self, use_center=use_center
            )
    ...
    def set_x_roi_is_valid(self, index: int, *, valid: bool):
        self.tablemodel.set_row_field_state(index, "STARTX", valid=valid)
