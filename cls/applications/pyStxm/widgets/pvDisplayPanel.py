...
class PVsPanel(QtWidgets.QWidget):
    ...
    def __init__(self, pv_dict, egu="", engineering_notation=False, parent=None):
        super(PVsPanel, self).__init__(parent)
        self.pv_dct = None
        self.pv_lst = None

        if type(pv_dict) is list:
            self.pv_lst = pv_dict
        else:
            self.pv_dct = pv_dict
        ...
        if self.pv_lst:
            ...
            for pv in self.pv_lst:
                ...
                desc = pv.get_desc()
                pv_ui = fbkClass(
                    pv, desc, fbk_val=fbk_val, engineering_notation=engineering_notation
                )
