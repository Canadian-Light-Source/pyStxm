...
_fbk_not_moving = master_colors["app_ltblue"]["rgb_str"]
...
class BasePanel(QtWidgets.QWidget):
    ...
    def __init__(
        self, positioner_set="ES", exclude_list=[], main_obj=None, parent=None
    ):
...
class PositionersPanel(BasePanel):
    """
    This is a widget that takes from the arg 'positioner_set' the positioners to filter from the
    master device list in the main object. This allows this Panel to present a panel of Endstation or Beamline
    positioners depending.
    ...
    def append_toggle_btn(self, name, desc, off_val, on_val, off_str, on_str, cb):
        ...
        pBtn.setStyleSheet(
            "QPushButton::indicator::checked{<b>%s</b>}		QPushButton::indicator::unchecked{<b>%s</b>}"
            % (on_str, off_str)
        )
...
class PositionerDetail(BasePanel, QtWidgets.QDialog):
    """
    classdocs
    """
...
def go():
    ...
    window2 = PositionersPanel("ES", [], main_obj=MAIN_OBJ)
