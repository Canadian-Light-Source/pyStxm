...
def TEST_set_devices_for_point_scan(gate, counter, shutter, type="point"):
    ...
    gate.trig_src_select.put(
        trig_src_pfi
    )  # /PFI 4  this will need to be part of a configuration at some point
    gate.retrig.put(1)
...
def set_devices_for_point_scan(
    scan_type, dwell, numE, numX, gate, counter, shutter=None
):
    """a convienience function to have a single place to configure the devices to acquire single points"""
...
def set_devices_for_line_scan(dwell, numX, gate, counter, shutter=None):
    ...
    gate.trig_src_select.put(
        trig_src_pfi
    )  # /PFI 3  connect PFI4 to the interferometer "pixelclock" wire
    ...
    counter.trig_src_select.put(
        trig_src_pfi
    )  # /PFI 3  connect PFI3 to the interferometer "pixelclock" wire
...
def calc_scan_velo(start, stop, dwell, npts, max_velo):
    ...
    if velo > max_velo:
        doprint(
            "info",
            "calc_velo: range=%.2f, dwell=%.2f, npts=%d" % (stop - start, dwell, npts),
        )
...
def calc_scan_npts(max_velo, rng, dwell):
    ...
    doprint(
        "info",
        "calc_scan_npts: max_velo=%.2f, range=%.2f um , dwell=%d"
        % (max_velo, rng, dwell),
    )
