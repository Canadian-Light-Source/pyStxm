...
class StxmDataToTif(QtCore.QObject):
    def __init__(self, fname, is_tmp_file=False):
        ...
        if is_tmp_file:
            ...
            self.num_images, self.rows, self.cols = self.nf["entry0"]["counter0"][
                "data"
            ].shape
    ...
    def process_tmp_file(self):
        ...
        for k in img_keys:
            ...
            if ("idx%s" % k) in list(nf_keys):
                ...
                ev = self.nf["idx%s" % k]["POSITIONERS"]["DNM_ENERGY"]["RBV"][()]
                ...
                self.save_tif(
                    fname + "_spid_%d" % sp_id + "_ev_%02f" % ev + "_%03d.tif" % int(k),
                    data,
                )
            else:
