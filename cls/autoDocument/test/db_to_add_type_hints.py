...
class TypeHintAdder(ast.NodeTransformer):
    allowed_types = {'int', 'float', 'str', 'list', 'dict', 'tuple', 'bool', 'set', 'frozenset', 'bytes', 'bytearray', 'complex'}
        ...
        if node.name == self.log_entry['function']:
            for arg in node.args.args:
                if arg.arg != 'self' and arg.arg in self.log_entry['arg_types']:
                    arg_type = self.log_entry['arg_types'][arg.arg]
                    if arg_type in self.allowed_types:
                        arg.annotation = ast.Name(id=arg_type, ctx=ast.Load())
            return_type = self.log_entry['return_type']
            if return_type in self.allowed_types:
