...
def main():
    ...
    p.add_option(
        ...
        action="store_true",
        dest="strip_text",
        help="Strip text for xml2json",
    )
    p.add_option(
        "--pretty",
        action="store_true",
        dest="pretty",
        help="Format JSON output so it is easier to read",
    )
    p.add_option(
        "--strip_namespace",
        action="store_true",
        dest="strip_ns",
        help="Strip namespace for xml2json",
    )
    p.add_option(
        "--strip_newlines",
        action="store_true",
        dest="strip_nl",
        help="Strip newlines for xml2json",
    )
            ...
            if k == "group":
                if isinstance(js[k], list):
                    for l in js[k]:
                        ...
                        dct = dict(
                            list(dct.items()) + list(walkit(js[k], del_str).items())
                        )
...
def nxdl_xml_to_json(class_dir, desired_class=None):
    ...
    for nxdl_file in nxdl_files:
        ...
        del_str_lst = [
            "{http://definition.nexusformat.org/nxdl/@NXDL_RELEASE@}",
