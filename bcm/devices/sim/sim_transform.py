...
class SimTransform(BaseSimObject):
    ...
    def __validrow(self, row):
        return isinstance(row, str) and len(row) == 1 and row in self.rows
    ...
    def gen_record_row(self, r, dct):
        r_dct = dct[r]
        s = (
            '\tfield(CMT%s, "%s")		field(INP%s, "%s")		field(CLC%s, "%s")		field(%s, "%s")		field(OUT%s, "%s")\n'
            % (
                r,
                r_dct["CMT%s" % r],
                r,
