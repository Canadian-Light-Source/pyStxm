...
class ophydPushBtn(QtWidgets.QPushButton):
    ...
    def __init__(
        ...
        self._dev_fbk_changed = self._signal_change
        if self.fbk_dev:
            # self.fbk_dev.add_callback(self._signal_change)
            self.fbk_dev.changed.connect(self._dev_fbk_changed)
        else:
            self.device.changed.connect(self._dev_fbk_changed)
    ...
    def _signal_change(self, dct):
        ...
        if self.update_counter > 0:
            self.update_counter -= 1
            #print(f'emiting changed[{dct[self.sig_change_kw]}]')
...
class ophydPushBtnWithFbk(QtWidgets.QPushButton):
    ...
    def __init__(
        ...
        if self.fbk_dev:
            ...
            self.fbk_dev.changed.connect(self._dev_fbk_changed)
            if self.fbk_dev.is_connected():
                val = int(self.fbk_dev.get("VAL"))
    ...
    def on_val_change(self, dct):
        ...
        #print(f"ophydPushBtnWithFbk: on_val_change: dct={dct}, self.sig_change_kw={self.sig_change_kw}, self.prefix={self.prefix}")
    ...
    def _signal_change(self, kw):
        ...
        :return:
        """
        # print('ophydPushButton.py: _signal_change: ophydPushBtnWithFbk: ', kw)
        dct = {}
...
class test(QtWidgets.QWidget):
    ...
    def __init__(self):
        ...
        # zp_inoutfbk_dev = Mbbi('BL1610-I10:uhv:zp_inout:fbk', val_only=True)
        # pv, init_val, off_val, on_val, off_str, on_str, cb=None):
        self.ev_en_pb = ophydPushBtnWithFbk(
            ev_en_dev,
            sig_change_kw="value",
            off_val=0,
            on_val=1,
        ...
        osa_y_dev = Bo(
            "BL1610-I10:ENERGY:uhv:osay_track_enabled", val_only=False, val_kw="value"
        )
        self.osa_y_dev_pb = ophydPushBtnWithFbk(
            osa_y_dev,
            sig_change_kw="value",
            off_val=0,
            on_val=1,
            off_str="OSAY Tracking Off",
            on_str="OSAY Tracking On",
            fbk_dev=osa_y_dev,
            toggle=True,
        )
        layout = QVBoxLayout()
