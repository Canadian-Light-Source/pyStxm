...
class EnergySelector(CurveViewerWidget):
    ...
    def __init__(
        self,
        winTitleStr="Energy Selector",
        toolbar=False,
        show_table=False,
        options={"gridparam": {"rgb_color": "rgb( 255, 255, 255)"}},
    ):
        super(EnergySelector, self).__init__(
            winTitleStr=winTitleStr, toolbar=toolbar, options=options
        )
    ...
    def zoom_to_range(self):
        ...
        y = np.zeros(len(x))
        self.set_xy_data("bckgrnd", x, y, update=False)
        self.set_autoscale()
