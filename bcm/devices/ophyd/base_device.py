...
from bcm.devices.zmq.zmq_device import ZMQSignal, ZMQSignalRO
    ...
    def __init__(
        ...
        **kwargs
        ...
        if backend.find("epics") == 0:
            ...
            #setup read/write pub sub connection here
            self._ophyd_dev = None
            self.write_pv = self.sig_name
        ...
        if backend.find("epics") > -1:
            ...
            print(f"signal_base: ERROR: backend [{backend}] should be using ZMQBaseDevice instead")
            exit()
        else:
            print("signal_base: ERROR: backend [%s] unsupported" % backend)
            exit()
